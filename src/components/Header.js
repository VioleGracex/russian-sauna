  ...
  return (
    <nav
      className={`fixed w-full top-0 z-50 transition-all duration-300 ${
        scrolled ? "bg-white shadow-md text-white " : "bg-transparent text-white "
      }`}
    >
      <div className="container mx-auto flex justify-between items-center py-4 px-6">
        ...
        <a href="/" className="flex items-center space-x-2">
          ...
          ${scrolled ? " text-black " : " text-white "}`}>Русская Баня</span>
        </a>
        ...
        <div className="md:hidden">
          <button
            ...
            className={`${scrolled ? " text-black " : " text-white "} text-2xl`}
          >
      ...
      {menuOpen && (
        <ul className="flex flex-col items-center bg-white bg-opacity-50 md:hidden space-y-4 py-4">
          {[
            { id: "#benefits", label: "Преимущества" },
            { id: "#services", label: "Услуги" },
            { id: "#reviews", label: "Отзывы" },
